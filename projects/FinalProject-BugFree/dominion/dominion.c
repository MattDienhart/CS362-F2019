...
int isGameOver(struct gameState *state) {
    ...
    j = 0;
    for (i = curse; i <= treasure_map; i++) // fixed Bug #4 - dienharm
    {
...
int scoreFor (int player, struct gameState *state) {
    ...
    //score from deck
    for (i = 0; i < state->deckCount[player]; i++) // changed from discardCount to deckCount to fix bug 5
    {
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case feast:
        ...
        //updateCoins(currentPlayer, state, 5); // removed this line to fix bug 6, along with the other change below
        ...
        while( x == 1) {//Buy one card
            ...
            else if (5 < getCost(choice1)) { // changed state->coins to 5 to fix bug 6, along with the other change above. 
                printf("That card is too expensive!\n");

                if (DEBUG) {
                    printf("Coins: %d < %d\n", *bonus, getCost(choice1));
                }
            }
            else {
    ...
    case mine:
        ...
        costLimit = getCost(state->hand[currentPlayer][choice1]) + 3;
        if (  getCost(choice2) > costLimit ) // fixed Bug #2 - dienharm
        {
            return -1;
        }
        ...
        for (i = 0; i < state->handCount[currentPlayer]; i++)
        {
            ...
            {
                discardCard(i, currentPlayer, state, 1); // fixed Bug #1 - dienharm
                break;
            }
        }
    ...
    case remodel:
        ...
        costLimit = getCost(state->hand[currentPlayer][choice1]) + 2;
        if (  getCost(choice2) > costLimit ) // fixed Bug #3 - dienharm
        {
            return -1;
        }
    ...
    case salvager:
        ...
        if (choice1)
        {
            ...
            *bonus = *bonus + getCost( handCard(choice1, state) );
            //trash card
...
int discardCard(int handPos, int currentPlayer, struct gameState *state, int trashFlag)
{
    ...
    if (trashFlag < 1)
    {
        // fixed as part of Bug #1 - dienharm
