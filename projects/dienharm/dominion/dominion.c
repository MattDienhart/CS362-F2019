...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case mine:
        return mineCardEffect(choice1, choice2, currentPlayer, state, handPos);
    ...
    case baron:
        return baronCardEffect(choice1, currentPlayer, state);
    ...
    case minion:
        return minionCardEffect(choice1, choice2, currentPlayer, state, handPos);
    ...
    case tribute:
        return tributeCardEffect(currentPlayer, state);

    case ambassador:
        return ambassadorCardEffect(choice1, choice2, currentPlayer, state, handPos);
...
int baronCardEffect(int choice, int currentPlayer, struct gameState *state)
{
   // choice > 0: discard an estate and gain 4 coins
   // choice <= 0: gain an estate
   ...
   if (choice > 0) { //Boolean true or going to discard an estate
     ...
     while(card_not_discarded) {
        if (state->hand[currentPlayer][p] == estate) { //Found an estate card!
           state->coins++;//Add 4 coins to the amount of coins
...
int minionCardEffect(int choice1, int choice2, int currentPlayer, struct gameState *state, int handPos)
{ 
   // choice1 = gain 2 coins
   // choice2 = discard hand and draw 4 cards, also force everyone with > 4 cards to discard their
...
int ambassadorCardEffect(int choice1, int choice2, int currentPlayer, struct gameState *state, int handPos)
{
   // choice1 = type of card to discard
   // choice2 = number of copies of that card to discard, must not exceed 2

   int j = 0;	//used to check if player has enough cards to discard

   // cannot remove more than 2 copies of the chosen card, otherwise the operation fails
   if (choice2 > 2 || choice2 < 0) {
      return -1;
   }

   // cannot remove the ambassador card itself, otherwise the operation fails
   if (choice1 == handPos) {
      return -1;
   }
   ...
   // cannot remove more copies of the "choice1" card than the player has, otherwise the operation fails
   if (j < choice2) {
      return -1;
   }
...
int tributeCardEffect(int currentPlayer, struct gameState *state)
{
   ...
   for (int i = 0; i <= 2; i ++) {
      ...
      else if (revealedCards[i] == estate || revealedCards[i] == duchy || revealedCards[i] == province || revealedCards[i] == gardens || revealedCards[i] == great_hall) { //Victory Card Found
         drawCard(currentPlayer, state);
         drawCard(currentPlayer, state);
      }
      else { //Action Card
...
int mineCardEffect(int choice1, int choice2, int currentPlayer, struct gameState *state, int handPos)
{
        ...
        // if selected card from supply is not a treasure card, the operation fails
        if (choice2 > treasure_map || choice2 < curse) {
            return -1;
        }

        // if the cost of the selected card is more than 3 + the cost of the discarded card,
        // the operation fials
